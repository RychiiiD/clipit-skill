...
4. 如 ffmpeg 未安装 → 调 clipit install
5. 告知用户安装完成，可开始使用
```
...
clipit transcribe <video_path> --model small > data/process-data/transcript.json
```
...
用 `data/process-data/transcript.json` 中的字幕数据构造 Prompt，调用 LLM 进行语义判断：
