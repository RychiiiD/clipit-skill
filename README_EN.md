...
**clipit** — Semantic video rough-cut tool. Say a topic, AI understands the content, keeps relevant segments, cuts off-topic chatter/repetition/pauses. The output is a complete broadcast script, not random sentence fragments.
...
- [Encoding Guard (Windows)](#encoding-guard-windows)
...
The Agent handles the full pipeline: transcribe → clean → analyze → validate → confirm → splice.
...
| `clipit validate <json> -i <intensity>` | Hard rule validation (R1-R6) | JSON |
...
The LLM does not classify sentences individually. It treats the transcript as source material for writing a complete broadcast script: opening hook → argument → evidence → case study → methodology → conclusion. Segments are paragraph-level (3-8 segments, each ≥5s).
